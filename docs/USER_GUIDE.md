...
### High-touch (default settings)
The agent asks for approval on every new position open and full exit. Good when:
- You're still building conviction in the system
- You want to review every entry decision before it executes
- You're running the system with a large portfolio
...
### Balanced (recommended after first month)
The agent opens new positions autonomously (journal already vetted them), but still asks before full exits.

```
gate_new_positions  →  OFF   (trust journal, let agent time the entry)
gate_full_exits     →  ON    (always confirm full liquidations)
approval_threshold  →  $2000 (routine scale-ins execute; large moves still ask)
```

### High-autonomy
The agent acts fully within its guardrails. You only get notified about outlier trades. Requires high confidence in your journal hygiene.
...
### Hard stops that always apply
Regardless of gate settings, these never turn off:
- If the portfolio is down 3% today → all trading stops for the rest of the session
- No single position can exceed 20% of the portfolio
- Maximum 5 trades executed per calendar day
- No buys within 7 days of earnings — automatic, no configuration needed
...
3. When a new disclosure appears, the agent calculates a proportional position based on the disclosed dollar range and your allocation
4. A pending decision card appears on the dashboard — you approve or reject

The allocation percentage is the maximum total exposure to that mirror source, not per trade. If Nancy Pelosi discloses 5 trades in a filing, the agent spreads your 3% across them proportionally.

Mirrors are additive — they surface buy candidates from an external signal, then still run through the same guardrails (blocklist, journal thesis gate, position size cap) before executing.
...
Set `daily_loss_halt_pct` to `0` — this will trigger the halt immediately and suspend all trading. Or set `max_trades_per_day` to `0`. To resume, restore the original values in Settings.
